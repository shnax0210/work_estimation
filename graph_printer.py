def find_pos(G, node, position, v_step, h_step, result):
    ...
    def find_weits_of_branshes(branches):
        def find_branch_size(branch, current_depth):
            branch_size = current_depth

            for sub_branch in G.successors(branch):
                branch_size = branch_size + find_branch_size(sub_branch, current_depth + 1)

            return branch_size

        branche_sizes = [find_branch_size(branch, 1) for branch in branches]
