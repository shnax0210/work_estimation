def calculate_requered_days(number_of_workers, all_tasks, graph, print_info=False):
    ...
    def find_free_task():
        all_free_tasks = [task for task in all_tasks if task[0] not in in_progress_tasks and task[0] not in completed_tasks]
    ...
    def get_diff(all_tasks, copleted_tasks):
        all_tasks = set([task[0] for task in all_tasks])
        return all_tasks - copleted_tasks
        
    while True:
        if print_info:
            print(in_progress_tasks)
            #print(get_diff(all_tasks, completed_tasks))
        days = days + 1
        update_workers()
